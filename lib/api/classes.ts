...
import type { Class, ClassChoice, CreateClassBody, UpdateClassBody } from "@/lib/api/schemas";
...
export const useUpdateClass = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ classId, body }: { classId: string; body: UpdateClassBody }) =>
...
export const useChooseClass = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, classId) => {
      queryClient.invalidateQueries({ queryKey: ["classes", classId, "choice"] });
